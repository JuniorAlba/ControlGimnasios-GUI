...
enum CriterioOrdencoachs { ORDEN_APENOMB, O_DNI, ORDEN_PLANES_RESPONSABLES, ORDEN_TELEFONO};
...
private:
	...
	std::string nombre_archivo_coachs;
	...
	std::vector<coach> vector_de_coachs;
...
public:
	...
	manage(std::string nombre_archivo_coachs, std::string nombre_archivo_clientes, std::string nombre_archivo_suscripciones, std::string nombre_archivo_planes);
	
	/// @brief Guarda los datos almacenados en los arreglos dentro de un archivo binario
	bool guardar();
	
	/// @brief Devuelve la cantidad de clientes, coachs, planes y suscripciones de los vectores correspondientes
	int cantidadCliente() const;
	int cantidadcoach() const;
	int cantidadPlanes() const;
	int cantidadSuscripciones() const;
	
	/// @brief Metodos para agregar clientes, coachs, planes y suscripciones a sus arreglos correspondientes
	void agregarCliente(const cliente &cl);
	void agregarcoach (const coach &ch);
	void agregarPlan(const plan &p);
	void agregarSuscripcion(const suscripcion &sub);
	
	/// @brief Métodos para borrar clientes, coachs, planes y suscripciones  de los arreglos correspondientes 
	void borrarCliente(int pos);
	void borrarcoach(int pos);
	void borrarPlan(int pos);
	void borrarSuscripcion(int pos);
	
	/// @brief Metodos para buscar un cliente o coach por su DNI
	cliente buscarClientesDNI(std::string DNI, int pos=0) const;
	coach buscarcoachsDNI(std::string DNI, int pos=0) const;
	
	/// @brief Metodo para buscar la posicion en la que se encuentra un coach por su DNI
	int buscarPoscoachsDNI(std::string DNI, int pos=0) const;
	...
	int buscarcoachsNombre(std::string nombre, int pos=0) const;
	int buscarcoachsNombre(int pos_plan, std::string nombre, int pos=0);
	...
	std::string planesResponsables(std::string dni_coach) const;
	
	/// @brief Metodos que nos permite saber cuales coachs estan o no en un plan (IN/OUT)
	std::vector<coach> coachsInPlan(int pos_plan);
	std::vector<coach> coachsOutPlan(int pos_plan);
	...
	void Ordenarcoachs(CriterioOrdencoachs criterio);
